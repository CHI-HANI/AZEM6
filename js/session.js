...
function slReorder(from, to) {
  ...
  if (from < sessIdx || to < sessIdx) { showMiniToast('⚠️ لا يمكن إعادة ترتيب تمرين مكتمل'); return; }
  const moved = sessExs.splice(from, 1)[0];
  sessExs.splice(to, 0, moved);
  buildSessList();
  buildSessTrack();
  showMiniToast('✅ تم إعادة الترتيب');
}
...
function sessSkipSet() {
  // Skip current set/rest only (old behavior of ⏭ button)
  const restScr = document.getElementById('sess-rest-scr');
  if (restScr && restScr.classList.contains('show')) {
    clearInterval(restTimerInterval);
    restScr.classList.remove('show');
...
function sessSkip() {
  // Skip entire current exercise → jump to next
  clearInterval(sessTimerInterval);
  clearInterval(restTimerInterval);
  const restScr = document.getElementById('sess-rest-scr');
