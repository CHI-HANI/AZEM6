  ...
  try {
    ...
    if (snap.exists()) {
      ...
      if (remote) {
        ...
        let merged;
        if (localTs > remoteTs) {
          // البيانات المحلية أحدث — احتفظ بها وخذ فقط ما ليس موجوداً محلياً
          merged = Object.assign({}, remote, S);
        } else {
          // البيانات السحابية أحدث — استخدمها
          merged = Object.assign({}, S, remote);
        }
        // دائماً خذ أعلى قيمة للسعرات والأيام
        ...
        // ✅ احتفظ بالصور المحلية دائماً — Firestore لا يحتفظ بها
        const localImages = S.customImages ? JSON.parse(JSON.stringify(S.customImages)) : {};
        Object.assign(S, merged);
        if (Object.keys(localImages).length > 0) {
          S.customImages = Object.assign({}, merged.customImages || {}, localImages);
        }
        saveState();
        try { render(); } catch(e) {}
        return true;
...
window.obFirebaseGoogleSignIn = async function() {
  ...
  try {
    ...
    const isStandalone = window.matchMedia('(display-mode: standalone)').matches
                      || window.navigator.standalone === true;
    // ✅ نضبط الـ flag قبل signInWithPopup لتجنب race condition مع onAuthStateChanged
    ...
    if (isStandalone) {
      ...
      if (result && result.user) {
        ...
        // ✅ نعيد الـ flag بعد تحديد المسار
    ...
    window._obGoogleJustSignedIn = false; // ✅ reset عند الخطأ
    if (btn) { btn.textContent = '🔵 تسجيل الدخول بـ Google'; btn.disabled = false; }
    if (e.code !== 'auth/popup-closed-by-user')
  ...
  if (user) {
    ...
    if (!window._obGoogleJustSignedIn) {
      ...
      if (!hasData && !S.onboardingDone) {
        // Firestore فارغ ولا توجد بيانات محلية → أعد الـ onboarding
        // ✅ تأكد أن الـ onboarding ليس مفتوحاً بالفعل لتجنب إعادة تشغيله
    ...
    // ✅ لا نعيد الـ flag هنا — يتم إعادته بعد انتهاء تدفق التسجيل كاملاً
  } else {
    _fbUid = null;
    window._fbUid = null;
    window._fbUser = null;
