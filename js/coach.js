...
function deRenderLibGrid() {
  ...
  grid.innerHTML = filtered.map(ex => {
    ...
    return `<div onclick="deLibAddEx('${ex.id}')" style="border-radius:16px;background:var(--card);border:2px solid ${borderColor};overflow:hidden;cursor:pointer;position:relative;aspect-ratio:1/1;-webkit-mask-image:-webkit-radial-gradient(white,black);">`
      + `<div style="width:100%;height:100%;background:${bgTint};position:relative;overflow:hidden;-webkit-mask-image:-webkit-radial-gradient(white,black);">`
      + mediaHTML + checkmark + badge
      + `<div style="position:absolute;bottom:0;left:0;right:0;padding:7px 8px;background:linear-gradient(to top,rgba(0,0,0,.75) 0%,transparent 100%);">`
      + `<div style="font-size:12px;font-weight:900;color:#fff;line-height:1.2;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">${ex.nameAr}</div>`
      + `<div style="font-size:9px;color:rgba(255,255,255,.7);margin-top:1px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">${ex.muscles || ex.nameEn || ''}</div>`
      + statusBtn
      + `</div>`
      + `</div></div>`;
  }).join('');
