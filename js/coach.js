...
العمر: ${u.age||'—'} سنة | الجنس: ${u.gender==='female'?'أنثى':u.gender==='male'?'ذكر':'—'}
الوزن: ${u.weight||'—'} كغ | الطول: ${u.height||'—'} سم | BMI: ${bmi} | السعرات الأساسية (BMR): ${bmr}
...
${(()=>{
  ...
  if (!thisWeek.length || !lastWeek.length) return '(لا يوجد بيانات كافية للمقارنة)';
  ...
  const trend = diff > 0 ? '📈 +' + pct + '% تحسن' : diff < 0 ? '📉 -' + pct + '% تراجع' : '➡️ مستقر';
  const consecutiveDays = (()=>{ let cnt=0; for(const e of allLog){if(e.exercises&&e.exercises.length)cnt++;else break;} return cnt; })();
  const restWarning = consecutiveDays >= 5 ? '⚠️ تحذير: ' + consecutiveDays + ' أيام متتالية بدون راحة — يجب الراحة!' : '';
  return 'هذا الأسبوع: ' + thisCalAvg + ' كال/جلسة | الأسبوع الماضي: ' + lastCalAvg + ' كال/جلسة | الاتجاه: ' + trend + (restWarning ? '\n' + restWarning : '');
})()}
...
- **ممنوع تكرار نفس الجملة أكثر من مرة في الرد**
...
- **لا تبدأ بكلمة "لا" أو نفي — ابدأ بالمساعدة مباشرة أو باسم المستخدم**

══════════════════════════════════
✍️ تنسيق الردود (مهم جداً):
══════════════════════════════════
- استخدم **نص** للعناوين والنقاط المهمة
- استخدم • لكل بند في القوائم
- اترك سطراً فارغاً بين كل قسم
- إيموجي واحد لكل قسم أو فكرة
- مثال صحيح للبرنامج:
**اليوم 1 — صدر:**
• ضغط 3×15
• بيربيز 3×10
...
🎯 قواعد FITCMD:
══════════════════════════════════
- عند تنفيذ أمر: اكتب ما فعلته بوضوح في ردّك — لا تكتب "تم التنفيذ" مجردة
- **عند إنشاء برنامج أسبوعي كامل:** أعرضه أولاً ثم اسأل "هل أطبقه على التطبيق؟" — انتظر موافقة قبل FITCMD
- **عند "طبق" أو "نفّذ" أو "ابدأ" أو "طبّق البرنامج":** نفّذ FITCMD فوراً بدون سؤال
- **استخدم فقط IDs من القائمة أدناه** عند setDay/setWeek/addExToDay — لا تخترع IDs ولا تستخدم أسماء بدل IDs
- setWeek يجب أن يحتوي 7 أيام بالضبط: [[...], [...], "rest", [...], [...], "rest", "rest"]
- **إذا أردت تمريناً غير موجود في القائمة:** أنشئه أولاً بـ addExercise ثم استخدم ID الجديد في setDay/setWeek
- بعد addExercise: أخبر المستخدم أنه يمكنه رفع صورة يدوياً من مكتبة التمارين
- **ممنوع استخدام ID وهمي أو اسم تمرين مكان ID** — هذا يُفشل التطبيق كاملاً

══════════════════════════════════
⚠️ قواعد اللغة والأسماء (حرجة جداً):
══════════════════════════════════
- **اكتب بالعربية الفصحى البسيطة فقط — ممنوع منعاً باتاً أي حرف من لغة أخرى داخل كلمة عربية**
- ممنوع: حروف يابانية، كورية، لاتينية داخل الكلمات العربية
- أسماء الأدوات الرياضية بالعربية الصحيحة: دمبل (وليس دنبل أو دَبَل)، بار، جهاز، حبل، كرة
- أسماء التمارين واضحة ومفهومة: "ضغط بالدمبل" وليس "الجلوس على المكتب"
- إذا لم تعرف الاسم العربي الصحيح لتمرين: استخدم وصفاً بسيطاً مثل "رفع الذراعين جانباً"
- تحقق من كل كلمة قبل الإرسال — لا أخطاء إملائية في أسماء الأدوات

══════════════════════════════════
🧠 ذكاء تكيّفي:
══════════════════════════════════
- إذا طلب "اقتراح جلسة" أو "الجلسة القادمة": حلّل سجل التدريب، نبّه لعضلة مُرهقة، واقترح تمارين متوازنة
- إذا كانت السعرات تنازلية: اقترح رفع الكثافة
- **إذا تدرب 5 أيام متتالية بدون راحة: نبّه بتحذير واضح ⚠️**
- قارن أسبوع هذا بالأسبوع الماضي عند الطلب
- تتبع الهدف: "أنت في الأسبوع X من هدفك، تقدمت Y%"
...
function coachExecCmd(cmd) {
  ...
  if (cmd.cmd === 'setDay' && cmd.day != null && Array.isArray(cmd.exercises)) {
    // لا ننفذ الامر اذا كانت قائمة التمارين فارغة تماما (خطا في الرد)
    if (cmd.exercises.length === 0) return false;
    // تحقق ان كل ID موجود فعلا في قاعدة التمارين
    ...
    S.customSchedule[Number(cmd.day)] = validExs;
